...
let main () =
  ...
  and dep_arg = ("-dep", Arg.String (fun s -> print_endline s; depends := Str.split (Str.regexp "[ \t]+") s), "specific opam dependencies [ocp-build]")
  ...
  in
  Arg.parse [ocp_arg; dep_arg; target_arg;url_arg; version_arg; keep_version_arg] (fun s -> ()) "";
  if !ocp_name = "" then
    (
	...
	(if Filename.is_relative archive_dir then
     ...
     in
      ...
      output_string opam_channel ("remove: [\n"^
                     "\t[make \"uninstall\"]\n"^
                     "]\n");
      output_string opam_channel (
		      Printf.sprintf"depends: [ %s %s ] \n"
				    (List.fold_left  (fun a b -> a^" \""^b^"\"") "" !depends)				     
				    (let s = ref "" in
				     List.iter (fun r -> 
						s := !s ^ 
